...
class iOSEngineerCodeCheckUITests: XCTestCase, Sendable {
    ...
    func testSearchAndDetailViewFlow() {
        XCTContext.runActivity(named: "SearchViewController 検索実行") { (activity) in
            app.launch()
        ...
        XCTContext.runActivity(named: "SearchViewController 検索結果タップ") { (activity) in
            // 検索結果が表示されるまで待機
        ...
        XCTContext.runActivity(named: "DetailViewController 詳細画面UI要素があるか") { (activity) in
        ...
        XCTContext.runActivity(named: "DetailViewController 戻るボタンを押して SearchViewController に戻る") { (activity) in
            let backButton = app.navigationBars.buttons.element(boundBy: 0)
            XCTAssertTrue(backButton.exists, "Back button should exist")
            backButton.tap()
        }
        
        XCTContext.runActivity(named: "DetailViewController 検索結果が保持されていることを確認") { (activity) in
            // MARK: - 4.
            // 検索画面が表示されるまで待機
